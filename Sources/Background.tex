...
To obtain facetted embedding we propose a method to extend the GloVe embedding to work on a named entity graph, namely LOAD. In LOAD the statistics of the corpus of a text is encoded in the edge weights between entities of type actors, location, organisation, date, and term. Edge weights can be used to generate a weighted adjacency matrix, which is comparable to a co-occurrence matrix in the GloVe model. \\
In this chapter, first, a brief overview of the text preprocessing steps for NLP tasks is given, in which methods used extract information from text and generate the LOAD network is explained. In Section~\ref{sec:nn}  neural network based approaches for learning embeddings is explain. 
Finally, to understand how the facetted embeddings are computed, it is important to have an overview of the GloVe model, which is described in Section~\ref{sec:GloVe}. 
