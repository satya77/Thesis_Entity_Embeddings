...
\begin{equation}
\label{eq:w2v_p}
w^*=\underset { w  }{ { \text{argmax} } } \prod _{ (w,c)\in \quad D }^{  }{ p(c|f; } w )
\end{equation}
...
\underset { w }{ \mathrm{ argmax } } \quad \sum _{ (c,a)\in D }^{  }{ \log { \sigma ( } w_{ c }.w_{ a }) } +\sum _{ (c,f)\in \acute { D }  }^{  }{ \log { \sigma ( } -w_{ c }.w_{ f })\quad  } 
\end{equation}
